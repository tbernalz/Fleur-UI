...
import LockhartUsersApi from '../../services/lockhartUsers';
...
export default function SignUpForm() {
  ...
  const handleEmailBlur = async () => {
    if (email.trim() !== '') {
      try {
        const { data } = await LockhartUsersApi.checkEmailExistence(email);
        if (data.exists) {
  ...
  const handleIdentificationBlurBlur = async () => {
    if (identificationNumber.trim() !== '') {
      try {
        const { data } = await LockhartUsersApi.checkIdentificationNumberExistence(identificationNumber);
        if (data.exists) {
  ...
  const handleSignUp = async () => {
    ...
    if (isValid) {
      ...
      try {
        ...
        await LockhartUsersApi.registerUser(userDto);
