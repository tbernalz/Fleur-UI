...
import MadEyeAuthApi from '../../services/madEyeAuth';
...
export default function PasswordResetForm() {
  ...
  const handleClick = async () => {
    ...
    if (isValid) {
      try {
        const response = await MadEyeAuthApi.requestPasswordReset({ email });
        const { email: resetEmail } = response.data; // Obtener el email de la respuesta
        enqueueSnackbar(`Se ha enviado un correo electrónico a ${resetEmail} para restablecer la contraseña`, {
          variant: 'success',
        });
        navigate('/');
      } catch (error) {
        setLoginError(
          'No se pudo solicitar el restablecimiento de contraseña. Por favor, inténtelo de nuevo más tarde.'
        );
      } finally {
