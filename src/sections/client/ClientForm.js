  ...
  const handleSubmit = async () => {
    ...
    if (isValid) {
      ...
      try {
        ...
        const signedUrlResponse = await dobbyFileManager.uploadFileSignedUrl(dataToSend);
        await dobbyFileManager.uploadFile(signedUrlResponse.content.url, fileInfo.mimeType, formData.file);
        ...
        enqueueSnackbar('Ocurrió un error al subir el archivo. Por favor, inténtalo de nuevo más tarde.', {
          variant: 'error',
        });
        console.log(error);
      }
        ...
        <Stack spacing={3}>
          {formData.isSuccess && (
            <Alert severity="success">
              <AlertTitle>{id ? 'Archivo editado con éxito' : 'Registro efectuado con éxito'}</AlertTitle>
              {id
                ? 'El Archivo se ha editado de forma exitosa con los nuevos valores'
