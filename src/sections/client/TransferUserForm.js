...
import thestralTransfer from '../../services/thestralTransfer';
...
export default function TransferUserForm() {
  ...
  useEffect(() => {
    const fetchOperators = async () => {
      try {
        const response = await thestralTransfer.getOperators();
    ...
    if (confirmed) {
      ...
      if (selectedOperator) {
        ...
        try {
          ...
          enqueueSnackbar('Transferencia realizada correctamente, en breve recibirá un correo electrónico.', {
            variant: 'success',
          });
        } catch (error) {
          console.error('Error al realizar la transferencia:', error);
          enqueueSnackbar('Error al realizar la transferencia. Por favor, inténtalo de nuevo más tarde.', {
            variant: 'error',
          });
        }
      } else {
        console.error('Operador seleccionado no encontrado:', formData.documentType);
        enqueueSnackbar('Error al encontrar el operador seleccionado. Por favor, selecciona un operador válido.', {
          variant: 'error',
        });
      }
    }
